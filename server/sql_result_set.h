...
class SQLResultSet {
	...
	struct SQLBlockInfo {
		...
		SQLBlockInfo(util::StackAllocator &alloc) :
				...
				varPart_(alloc),
				fixedPartOut_(
						util::ByteStream< util::XArrayOutStream<> >(
	...
	public:
		...
		template <typename T>
		void setFixedValueWithPadding(
				ColumnType type, const void* value) {
			
			(*cursor_.fixedPartOut_) << type;
			(*cursor_.fixedPartOut_).writeAll(value, sizeof(T));
			(*cursor_.fixedPartOut_).writeAll(
					paddingBuffer_, sizeof(int64_t) - sizeof(T));
		}

		template <typename T>
		void setFixedValue(ColumnType type, const void* value) {
		...
		void setVariable(
			...
			cursor_.baseVarOffset_
					+= (cursor_.varPartOut_->base().position() - startPos);
		}

		template <typename T>
		void setNull(ColumnType type) {
			(*cursor_.fixedPartOut_) << COLUMN_TYPE_NULL;
		}
		...
		void getColumnNameList(
				util::StackAllocator &alloc,
				util::Vector<util::String> &columnNameList) {

			for (size_t pos = 0;
					pos < columnNameList_.size(); pos++) {
				columnNameList.push_back(
						util::String(columnNameList_[pos].c_str(), alloc));
			}
		}

		void setColumnNameList(
				util::Vector<util::String> &columnNameList) {
	...
	private:
		...
		util::AllocVector<SQLBlockInfo*> blockList_;
		util::AllocVector<SQLString> columnNameList_;
		util::AllocVector<ColumnType> columnTypeList_;
		SQLCursor cursor_;
