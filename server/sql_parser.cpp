...
SyntaxTree::Expr& SyntaxTree::Expr::operator=(const Expr &another) {
	...
	patternRecognizeOpt_ = another.patternRecognizeOpt_, 
...
SyntaxTree::RegexNode* SyntaxTree::RegexNode::makeRegexNode(
		SQLAllocator &alloc, SyntaxTree::PatternRegexNodeType type,
		SyntaxTree::RegexNode* arg1, SyntaxTree::RegexNode* arg2) {
	SyntaxTree::RegexNode* node = ALLOC_NEW(alloc) SyntaxTree::RegexNode(alloc);
	node->nodeType_ = type;
	if (arg1) {
		node->children_.push_back(arg1);
	}
	if (arg2) {
		node->children_.push_back(arg2);
	}
	return node;
}
...
SyntaxTree::Expr* SyntaxTree::Expr::duplicate(SQLAllocator &alloc) const {
	...
	try {
		...
		expr->patternRecognizeOpt_ = patternRecognizeOpt_; 
...
void SyntaxTree::Expr::dump(std::ostream &os) {
	...
	if (patternRecognizeOpt_) {
		os << ", \"patternRecognizeOption\":";
		patternRecognizeOpt_->dump(os);
	}
	if (left_) {
		os << ", \"left\":";
		left_->dump(os);
...
SyntaxTree::PatternRecognizeOption::PatternRecognizeOption(SQLAllocator &alloc):
		alloc_(alloc),
		partitionByList_(NULL),
		orderByList_(NULL),
		measuresList_(NULL),
		matchMode_(PATERN_MATCH_ONE_ROW_PER_MATCH),
		skipToMode_(PATTERN_SKIP_PAST_LAST_ROW),
		pattern_(NULL),
		defineList_(NULL) {
}

void SyntaxTree::PatternRecognizeOption::dump(std::ostream &os) {
	os << "{\"PatternRecognizeOption\":{";
	os << ", \"matchMode:" << static_cast<int32_t>(matchMode_);
	if (partitionByList_) {
		os << ", \"partitionByList\":[";
		ExprList::iterator itr = partitionByList_->begin();
		for (; itr != partitionByList_->end(); ++itr) {
	...
	if (measuresList_) {
		os << ", \"measuresList\":[";
		ExprList::iterator itr = measuresList_->begin();
		for (; itr != measuresList_->end(); ++itr) {
			if (itr != measuresList_->begin()) {
				os << ", ";
			}
			if (*itr) {
				(*itr)->dump(os);
	...
	if (pattern_) {
		os << ", \"patternExpr\":{";
		pattern_->dumpTree(os, 0);
		os << "}";
	}
	if (defineList_) {
		os << ", \"defineList\":[";
		ExprList::iterator itr = defineList_->begin();
		for (; itr != defineList_->end(); ++itr) {
			if (itr != defineList_->begin()) {
				os << ", ";
			}
			if (*itr) {
				(*itr)->dump(os);
...
SyntaxTree::RegexNode::RegexNode(SQLAllocator &alloc):
		alloc_(alloc),
		nodeType_(PATTERN_REGEX_NONE),
		children_(alloc), varName_(NULL), min_(-999), max_(-999)
{
}

SyntaxTree::RegexNode::RegexNode(
		SQLAllocator &alloc, PatternRegexNodeType type,
		RegexNode* arg1, RegexNode* arg2) :
	alloc_(alloc),
	nodeType_(type),
	children_(alloc), varName_(NULL), min_(-999), max_(-999)
{
	if (arg1) {
		children_.push_back(arg1);
	}
	if (arg2) {
		children_.push_back(arg2);
	}
}


void SyntaxTree::RegexNode::dumpTree(std::ostream& os, int indent) const {
	os << "{";
	os << "\"nodeType\": \"" << nodeTypeToString(nodeType_) << "\"," << std::endl;
	if (varName_ && varName_->name_) {
		os << "\"varName\": \"" << *varName_->name_ << "\"," << std::endl;
	}
	if (min_ != -999) {
	...
	os << "\"children\": [";
	if (!children_.empty()) os << std::endl;
	for (size_t i = 0; i < children_.size(); ++i) {
		if (children_[i]) {
			children_[i]->dumpTree(os, indent + 2);
...
const char* SyntaxTree::RegexNode::nodeTypeToString(PatternRegexNodeType type) {
	switch (type) {
	case PATTERN_REGEX_NONE:         return "PATTERN_REGEX_NONE";
	case PATTERN_REGEX_ALT:          return "PATTERN_REGEX_ALT";
	case PATTERN_REGEX_CONCAT:       return "PATTERN_REGEX_CONCAT";
	case PATTERN_REGEX_STAR:         return "PATTERN_REGEX_STAR";
	case PATTERN_REGEX_PLUS:         return "PATTERN_REGEX_PLUS";
	case PATTERN_REGEX_QMARK:        return "PATTERN_REGEX_QMARK";
	case PATTERN_REGEX_REPEAT:       return "PATTERN_REGEX_REPEAT";
	case PATTERN_REGEX_GROUP:        return "PATTERN_REGEX_GROUP";
	case PATTERN_REGEX_ANCHOR_START: return "PATTERN_REGEX_ANCHOR_START";
	case PATTERN_REGEX_ANCHOR_END:   return "PATTERN_REGEX_ANCHOR_END";
	case PATTERN_REGEX_VARNAME:      return "PATTERN_REGEX_VARNAME";
	default:                         return "PATTERN_REGEX_UNKNOWN";
	}
}
...
void SQLIndexStatsCache::Body::putSub(
	...
	if (it == keyMap_.end()) {
		...
		it = keyMap_.insert(
				std::make_pair(keyHolder.get(), Entry(freeList_.end()))).first;
		keyHolder.detach();
	}
	else {
