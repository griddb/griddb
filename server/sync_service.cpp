...
void LongTermSyncHandler::operator()(EventContext &ec, Event &ev) {
	...
	try {
		...
		case TXN_LONGTERM_SYNC_REQUEST: {
			...
			addLongtermSyncCheckEvent(ec, pId, alloc, context, LONG_TERM_SYNC_OWNER);
		...
		TRACE_SYNC_EXCEPTION(e, eventType, pId, WARNING,
				"Long term sync operation is failed. reason=" << GS_EXCEPTION_MESSAGE(e));
		...
		if (context != NULL) {
			if (errorCode != GS_ERROR_SYM_INVALID_PARTITION_REVISION
				...
				UTIL_TRACE_EXCEPTION(SYNC_SERVICE, e, "LONGTERM SYNC FAILED, pId=" << pId 
					...
					<< ", revision=" << context->getPartitionRevision().sequentialNumber_ 
					<< ", event=" << getEventTypeName(eventType) << ", reason=" << GS_EXCEPTION_MESSAGE(e));
...
void SyncCheckEndHandler::operator()(EventContext &ec, Event &ev) {
	...
	try {
		...
		if (context != NULL) {
			...
			if (in.base().remaining() > 0) {
				...
				if (context->checkLongTermSync(
						syncMgr_->getConfig().getLongtermCheckIntervalCount())) {
			...
			if (context->getSyncMode() == MODE_SHORTTERM_SYNC && 
				...
				if (context->checkTotalTime(
					...
					GS_TRACE_SYNC(context, "Short term sync error", ", check previous trace," 
		...
		UTIL_TRACE_EXCEPTION_WARNING(SYNC_SERVICE, e, "reason=" << GS_EXCEPTION_MESSAGE(e));
		if (context) {
			syncMgr_->removeSyncContext(ec, pId, context, true);
		}
	}
...
void RecvSyncMessageHandler::operator()(EventContext &ec, Event &ev) {
	...
	try {
		...
		TRACE_SYNC_EXCEPTION(
				e, syncEventType, pId, ERROR, "Recv sync message failed, ");
	}
	catch (std::exception &e) {
		clsSvc_->setError(ec, &e);
	}
...
void SyncHandler::addLongtermSyncCheckEvent(EventContext &ec, PartitionId pId,
	util::StackAllocator &alloc, SyncContext *context, int32_t longtermSyncType) {
	try {
		int32_t timeoutInterval = clsMgr_->getConfig().getCheckLoadBalanceInterval();
		Event syncCheckEndEvent(ec, TXN_SYNC_TIMEOUT, pId);
		SyncCheckEndInfo syncCheckEndInfo(
				alloc, TXN_SYNC_TIMEOUT, syncMgr_, pId, MODE_LONGTERM_SYNC, context);
		EventByteOutStream out = syncCheckEndEvent.getOutStream();
		syncSvc_->encode(syncCheckEndEvent, syncCheckEndInfo, out);
...
void ShortTermSyncHandler::decodeLongSyncRequestInfo(
	...
	if (in.base().remaining()) {
		...
		GS_TRACE_SYNC_NORAML("[OWNER] Catchup promotion sync start, pId="
...
void LongTermSyncHandler::requestShortTermSync(
	...
	GS_TRACE_SYNC(context, 
			"[OWNER] Request promoted partition short term sync", ", role=" << nextRole);
...
void LongTermSyncHandler::executeSyncStart(EventContext &ec, Event &ev, PartitionId pId,
	...
	addLongtermSyncCheckEvent(ec, pId, ec.getAllocator(),
			context, LONG_TERM_SYNC_CATCHUP);
