...
void SyncService::initialize(ResourceSet &resourceSet) {

	clsSvc_ = resourceSet.clsSvc_;
	txnSvc_ = resourceSet.txnSvc_;
	logMgr_ = resourceSet.logMgr_;
	syncMgr_->initialize(&resourceSet);
	recvSyncMessageHandler_.initialize(resourceSet);
...
void SyncHandler::initialize(const ResourceSet &resourceSet) {
...
void SyncHandler::sendEvent(
		EventContext &ec, EventEngine &ee,
		NodeId targetNodeId,
		Event &ev,
		int32_t delayTime) {
...
void ShortTermSyncHandler::operator()(
	...
	try {
		...
		TRACE_SYNC_EXCEPTION(e, eventType, pId, WARNING,
				"Short term sync retried by lock cofilct");
...
bool SyncRequestInfo::getLogs(
	...
	try {
		...
		while (cursor.nextLog(
				logRecord, binaryLogRecords_, pId) &&
				logRecord.lsn_ <= request_.endLsn_ &&
				binaryLogRecords_.size()
						<= condition_.logSize_) {
...
bool SyncRequestInfo::getChunks(
	...
	try {
		...
		for (int32_t pos = 0;
				pos < static_cast<int32_t>(condition_.chunkNum_);pos++) {

			uint8_t *chunk = reinterpret_cast<uint8_t*>(
					syncMgr_->getChunkBuffer(pgId, pos));
		...
		if (lastChunkGet) {
			LogCursor cursor;
			if (!cpSvc->getLongSyncCheckpointStartLog(
					sId, cursor)) {
				GS_THROW_USER_ERROR(
						GS_ERROR_SYNC_CHUNK_GET_FAILED, 
						"Long term sync get checkpoint start log failed, pId="
						<< pId);
			}
	...
	try {
		...
		DataStore::Latch latch(txn, pId, ds_, clsSvc_);
		{
			...
			TRACE_CLUSTER_NORMAL_OPERATION(
					INFO,
					"Drop partition completed (pId=" << pId
					<< ", LSN=" << prevLsn << ")");
		}
		clsSvc_->requestChangePartitionStatus(
				ec, alloc, pId,
...
void SyncManagerInfo::decode(
	...
	uint32_t lastSize = static_cast<uint32_t>(in.base().position()) + bodySize;
...
*/
void SyncHandler::removePartition(
		util::StackAllocator &alloc,
		PartitionId pId,
		EventType eventType,
		const util::DateTime &now,
...
void ShortTermSyncHandler::decodeLongSyncRequestInfo(
	...
	if (in.base().remaining()) {
		...
		GS_TRACE_SYNC_NORAML(
				"[OWNER] Catchup promotion sync start, pId="
				<< pId 
				<< ", syncId=" << longSyncId.dump());
...
void SyncHandler::checkRestored(
		util::StackAllocator &alloc,
		PartitionId pId,
		EventType eventType,
		const util::DateTime &now,
