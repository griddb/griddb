node {
    stage('CentOS 6.8 Build') {
        docker.image('centos:6.8_dev').inside {
            ...
            sh './bootstrap.sh; ./configure; make; make dist-zip; cp griddb-3.0.zip installer/SOURCES/; cd installer; rpmbuild --define="_topdir `pwd`" -bb --clean SPECS/griddb.spec; rm -rf *.o'
        }
    }
